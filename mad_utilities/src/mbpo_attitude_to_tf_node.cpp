...
void imuMsgCallback( const sensor_msgs::Imu& imu_msg ){
  ...
  transform_.stamp_ = imu_msg.header.stamp;//ros::Time::now();

  tfB_->sendTransform( transform_ );
}

int main(int argc, char **argv) {
  ros::init( argc,argv,"mbpo_attitude_to_tf_node" );
  ...
  ros::Subscriber imu_subscriber = n.subscribe( "mavros/imu/data",10,imuMsgCallback );
